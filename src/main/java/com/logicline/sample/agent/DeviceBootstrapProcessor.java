...
public class DeviceBootstrapProcessor {
  ...
  public CumulocityCredentials process(String serialNumber) {
    ...
    DeviceCredentialsRepresentation credentialsRepresentation = deviceCredentialsApi
        .pollCredentials(serialNumber, POLL_CREDENTIALS_INTERVAL, POLL_CREDENTIALS_TIMEOUT);
    if (credentialsRepresentation == null) {
      logger.info("No credentials accessed for serialNumber {}.", serialNumber);
      return null;
    }
    CumulocityCredentials cumulocityCredentials =
        asCumulocityCredentials(credentialsRepresentation);
    credentialsManager.saveDeviceCredentials(cumulocityCredentials);
    return cumulocityCredentials;
  }

  private CumulocityCredentials asCumulocityCredentials(
      DeviceCredentialsRepresentation credentialsRepresentation) {
    return cumulocityCredentials(credentialsRepresentation.getUsername(),
  ...
  private DeviceCredentialsApi getDeviceCredentialsApi() {
    return new PlatformImpl(credentialsManager.getHost(),
        credentialsManager.getBootstrapCredentials()).getDeviceCredentialsApi();
  }
